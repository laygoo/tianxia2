	...
	: QMainWindow(parent)
{
	...
	//QRegExp regx("[0-9]+$");
	//QValidator *validator = new QRegExpValidator(regx, ui.forceIdField);
	//ui.forceIdField->setValidator(validator);
	//this->openTheEngine();

	TianxiaSingleton* tx = TianxiaSingleton::getInstance();
	tx->table = ui.tableWidget;
	//tx->forceIdField = ui.forceIdField;
	...
	QObject::connect(ui.createForceBtn, SIGNAL(clicked()), this, SLOT(createForceBtnClicked()));
	QObject::connect(ui.tuizuBtn, SIGNAL(clicked()), this, SLOT(tuizuBtnClicked()));
	...
	QObject::connect(ui.weimaBtn, SIGNAL(clicked()), this, SLOT(weimaBtnClicked()));
	...
	QObject::connect(ui.backMenpaiBtn, SIGNAL(clicked()), this, SLOT(backMenpaiBtnClicked()));
	QObject::connect(ui.kuailvBtn, SIGNAL(clicked()), this, SLOT(kuailvBtnClicked()));
	QObject::connect(ui.wuyaoBtn, SIGNAL(clicked()), this, SLOT(wuyaoBtnClicked()));
	QObject::connect(ui.submitLiuliBtn, SIGNAL(clicked()), this, SLOT(submitLiuliBtnClicked()));
	...
	QObject::connect(ui.liuguangBtn, SIGNAL(clicked()), this, SLOT(liuguangBtnClicked()));
...
//createForceBtnClicked
void ControllerFrameEx::createForceBtnClicked() {
	qDebug() << "createForceBtnClicked";
	ui.scriptSortList->addItem(QString::fromLocal8Bit("CJSL"));
	ui.scriptSortList->setCurrentRow(ui.scriptSortList->count() - 1);
}
//tuizuBtnClicked
void ControllerFrameEx::tuizuBtnClicked() {
	qDebug() << "tuizuBtnClicked";
	ui.scriptSortList->addItem(QString::fromLocal8Bit("TZ"));
	ui.scriptSortList->setCurrentRow(ui.scriptSortList->count() - 1);
}
...
//weimaBtnClicked
void ControllerFrameEx::weimaBtnClicked() {
	qDebug() << "weimaBtnClicked";
	ui.scriptSortList->addItem(QString::fromLocal8Bit("WMA"));
	ui.scriptSortList->setCurrentRow(ui.scriptSortList->count() - 1);
}
...
//liuguangBtnClicked
void ControllerFrameEx::liuguangBtnClicked() {
	qDebug() << "liuguangBtnClicked";
	ui.scriptSortList->addItem(QString::fromLocal8Bit("LGJZ"));
	ui.scriptSortList->setCurrentRow(ui.scriptSortList->count() - 1);
}
...
//backMenpaiBtnClicked
void ControllerFrameEx::backMenpaiBtnClicked() {
	qDebug() << "backMenpaiBtnClicked";
	ui.scriptSortList->addItem(QString::fromLocal8Bit("HDMP"));
	ui.scriptSortList->setCurrentRow(ui.scriptSortList->count() - 1);
}
//kuailvBtnClicked
void ControllerFrameEx::kuailvBtnClicked() {
	qDebug() << "kuailvBtnClicked";
	ui.scriptSortList->addItem(QString::fromLocal8Bit("KLJB"));
	ui.scriptSortList->setCurrentRow(ui.scriptSortList->count() - 1);
}

void ControllerFrameEx::wuyaoBtnClicked() {
	qDebug() << "wuyaoBtnClicked";
	ui.scriptSortList->addItem(QString::fromLocal8Bit("WYP"));
	ui.scriptSortList->setCurrentRow(ui.scriptSortList->count() - 1);
}
//submitLiuliBtnClicked
void ControllerFrameEx::submitLiuliBtnClicked() {
	qDebug() << "submitLiuliBtnClicked";
	ui.scriptSortList->addItem(QString::fromLocal8Bit("CHLL"));
	ui.scriptSortList->setCurrentRow(ui.scriptSortList->count() - 1);
}
...
void ControllerFrameEx::generalLoadConfig(QString file) {
	...
	int forceIdField = configIniRead.value("/script_config/forceIdField").toInt();
	ui.forceIdField->setValue(forceIdField);
...
QString ControllerFrameEx::loadScriptConfig() {
	...
	json.insert("forceIdField", ui.forceIdField->text());
...
void ControllerFrameEx::generalConfiguration(QString file) {
	...
	//int forceIdFieldStr = ui.forceIdField->value();
	settings.setValue("forceIdField", ui.forceIdField->value());
...
void ControllerFrameEx::onRightClicked(QPoint point) {
	...
	menu.addAction(QString::fromLocal8Bit("窗口截图"), this, SLOT(takeScreenShot()));
...
void ControllerFrameEx::takeScreenShot() {
	qDebug() << "standarlizeWnd";
	int a = ui.tableWidget->currentColumn();
	int b = ui.tableWidget->currentRow();
	...
	int bindRet = DM->PskBindWndEx(tHWND.toLong(), "dx2", "windows", "windows", "", 103);
	...
	string path;
	path.assign(buf);
	path.append("\\tianxiaReport.jpg");

	DM->PskCapture(0, 0, 600, 400, path.c_str());
	DM->PskUnBindWnd();
	DM->Release();
}

Ifire* ControllerFrameEx::initDM() {
	Ifire* DM = DMSoft::getInstance()->newDMObject();
	qDebug() << DM->PskVer();
