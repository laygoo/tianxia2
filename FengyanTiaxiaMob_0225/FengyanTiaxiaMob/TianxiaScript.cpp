...
{
	if (findDeadBlood(DM) == 1) {
		int counter = 0;
		while (getIsDead(DM) == 0 && getIsEnded() == false) {
			Sleep(500);
			/*DM->PskMToEx(300, 200, 10, 10);
			DM->PskLClick();*/
			counter++;
			if (counter > 120) {
				break;
...
int TianxiaScript::yuandiFuhuoPic(Ifire* DM)
{
	...
	bstr = DM->PskFindPicE(220, 200, 350, 300, "YuandiFuhuo2.bmp|YuandiFuhuoBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::findDeadBlood(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(30, 0, 150, 100, "DeadBlood.bmp|DeadBlood2.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "DeadBloodX=" << result[1].c_str();
	qDebug() << "DeadBloodY=" << result[2].c_str();
...
int TianxiaScript::getIsDead(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(250, 10, 450, 120, "IsDead1.bmp|IsDead2.bmp|IsDead3.bmp|IsDead4.bmp|IsDead5.bmp|IsDead6.bmp|IsDead7.bmp|IsDead8.bmp|IsDead9.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	bstr = DM->PskFindPicE(200, 10, 550, 120, "Dead1.bmp|Dead2.bmp|Dead3.bmp|Dead4.bmp|Dead5.bmp|Dead7.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::duihuaHuangtiao(Ifire* DM)
{
	...
	bstr = DM->PskFindPicE(10, 300, 450, 360, "DuiHuaTiao.bmp|DuihuaHuangtiao.bmp|DuihuaHuangtiao2.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::ShizhuangCloseBtnClose(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(510, 0, 600, 120, "ShizhuangCloseBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "ShizhuangCloseBtnX=" << result[1].c_str();
	qDebug() << "ShizhuangCloseBtnY=" << result[2].c_str();
...
int TianxiaScript::clickFuyunCloseBtn(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(360, 15, 480, 160, "FuyunCloseBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "FuyunCloseBtnX=" << result[1].c_str();
	qDebug() << "FuyunCloseBtnY=" << result[2].c_str();
...
int TianxiaScript::clickJijinCloseBtn(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(500, 15, 650, 160, "JijinCloseBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "JijinCloseBtnX=" << result[1].c_str();
	qDebug() << "JijinCloseBtnY=" << result[2].c_str();
...
int TianxiaScript::getIsEquiped(Ifire* DM) {
	...
	if (findFuliWnd(DM) == 1) {
		...
		int temp = 0;
		while (clickYouxiaZhuangbeiBtn(DM) == 1 && getIsEnded() == false) {
			temp++;
			if (temp > 25) {
				break;
			}
...
int TianxiaScript::zhuxianSubScript(Ifire* DM)
{
	...
	if (currentLevel > 20 && currentLevel < 31 && this->isEquiped == false) {
		getIsEquiped(DM);
	}
...
int TianxiaScript::clickHuodongCloseBtn(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(450, 10, 600, 100, "HuodongCloseBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "HuodongCloseBtnX=" << result[1].c_str();
	qDebug() << "HuodongCloseBtnY=" << result[2].c_str();
...
int TianxiaScript::findPetSkill(Ifire* DM) {
	...
	std::vector<std::string> result;
	//bstr = DM->PskFindPicE(130, 250, 200, 330, "摊位费.bmp|Tanweifei.bmp", "000000", 0.9, 0);
	bstr = DM->PskFindPicE(180, 120, 280, 220, "PetSkill.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "PetSkillX=" << result[1].c_str();
	qDebug() << "PetSkillY=" << result[2].c_str();
...
int TianxiaScript::findShoujiaTitle(Ifire* DM) {
	...
	std::vector<std::string> result;
	//bstr = DM->PskFindPicE(130, 250, 200, 330, "摊位费.bmp|Tanweifei.bmp", "000000", 0.9, 0);
	bstr = DM->PskFindPicE(180, 200, 300, 300, "ShoujiaTitle.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::findShangchengWnd(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(250, 20, 350, 100, "商城窗口.bmp|ShangchengWnd.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::getCurrentHuoyue(Ifire* DM)
{
	...
	DM->PskSetDict(0, "huoli_zk.txt");
	//5E4327-2F2E25
	//bstr = DM->PskOcr(80, 300, 130, 320, "60492F-2C2E28|806340-1D1D17", 1);
	bstr = DM->PskOcr(80, 285, 130, 320, "874632-1C291A", 0.9);//874632-1C291A
...
int TianxiaScript::getCurrentJunzi(Ifire* DM)
{
	...
	bstr = DM->PskOcr(330, 295, 390, 315, "9E642F-192829|D759BC-06271E|448ED9-3B1320|ED7D35-0D1C25", 1);
	string leverString = (_bstr_t)bstr;
...
int TianxiaScript::getSellingPrice(Ifire* DM)
{
	...
	bstr = DM->PskOcr(360, 130, 450, 160, "EB7C33-0E1923|ED7322-0D1419|A66C30-1A2721|4A8ED0-2D0D1C", 1);
	string leverString = (_bstr_t)bstr;
...
int TianxiaScript::menpaiBiwuNew(Ifire* DM)
{
	...
	menpaiBiwuSubScriptNew(DM);
	//this->hideTheOthers(DM);
	this->isHided = false;
...
int TianxiaScript::juediQiusheng(Ifire* DM)
{
	...
	while (juediQiushengSubScriptEx(DM) == 0 && getIsEnded() == false) {
		Sleep(2000);
		temp++;
		if (temp > 30) {
			isValidated = false;
	...
	if (isValidated) {
		temp = 0;
		while (clickConfirmBtn(DM) == 0 && getIsEnded() == false) {
			Sleep(2000);
			temp++;
			if (temp > 30) {
				break;
		...
		while (juediEnd(DM) == 0 && getIsEnded() == false) {
			//Sleep(10000);
			DM->PskKDown(87);
			Sleep(10000);
			DM->PskKPress(32);
...
int TianxiaScript::juediQiushengSubScriptEx(Ifire* DM) {
	if (clickDanrenpaiduiBtn(DM) == 0) {
		if (clickJiutianBtn(DM) == 1) {
			//DM->PskKPress(27);
			clickHuodongCloseBtn(DM);
			Sleep(1000);
			clickJiutianCansaiBtn(DM);
		}
		else {
			clickJiutianCansaiBtn(DM);
		}
	...
	Sleep(2000);
	/*while (findCaptchaPic(DM, pX, pY) == 0 && getIsEnded() == false) {
		DM->PskMTo(390, 140);
		DM->PskLClick();
		Sleep(1000);
	}*/
	...
	moveScrollBtn(DM, pX);
	Sleep(2000);
	if (findPipeiSuccess(DM) == 1|| clickConfirmBtn(DM) == 1) {
		Sleep(5000);
		return 1;
	}
...
int TianxiaScript::clickConfirmBtn(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(300, 200, 400, 380, "ConfirmBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "ConfirmBtnX=" << result[1].c_str();
	qDebug() << "ConfirmBtnY=" << result[2].c_str();
...
int TianxiaScript::menpaiPanelBTN(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(400, 250, 550, 360, "MenpaiPanelBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "MenpaiPanelBtnX=" << result[1].c_str();
	qDebug() << "MenpaiPanelBtnY=" << result[2].c_str();
...
int TianxiaScript::findChijiMap(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(80, 100, 200, 250, "ChijiMap.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::findTianyuanMap(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(220, 60, 360, 180, "TianyuanMap.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "TianyuanMapX=" << result[1].c_str();
	qDebug() << "TianyuanMapY=" << result[2].c_str();
...
int TianxiaScript::clickDanrenpaiduiBtn(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(400, 250, 550, 350, "DanrenpaiduiBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "DanrenpaiduiBtnX=" << result[1].c_str();
	qDebug() << "DanrenpaiduiBtnY=" << result[2].c_str();
...
int TianxiaScript::clickJiutianBtn(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(400, 250, 550, 350, "JiutianBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "JiutianBtnX=" << result[1].c_str();
	qDebug() << "JiutianBtnY=" << result[2].c_str();
...
int TianxiaScript::clickJiutianCansaiBtn(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(400, 250, 550, 350, "JiutianCansaiBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "JiutianCansaiBtnX=" << result[1].c_str();
	qDebug() << "JiutianCansaiBtnY=" << result[2].c_str();
...
int TianxiaScript::findPipeiSuccess(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(200, 0, 350, 100, "PipeiSuccess.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "PipeiSuccessX=" << result[1].c_str();
	qDebug() << "PipeiSuccessY=" << result[2].c_str();
...
int TianxiaScript::moveScrollBtn(Ifire* DM, long pX)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(180, 220, 410, 300, "ScrollBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "ScrollBtnX=" << result[1].c_str();
	qDebug() << "ScrollBtnY=" << result[2].c_str();
	...
	if (x != -1) {
		...
		int dis = pX - x;
		for (int i = 0; i < dis / 15; i++) {
			DM->PskMToEx(x + i * 15, y + 2, 5, 5);
			Sleep(200);
		}

		DM->PskMToEx(pX + 15, y + 2, 5, 5);
		DM->PskLUp();
		Sleep(1000);
		return 1;
	}
...
int TianxiaScript::findCaptchaPic(Ifire* DM, int &x, int &y)
{
	...
	std::vector<std::string> result;
	string pics = "";
	string captcha1 = "CaptchaPic2.bmp|CaptchaPic.bmp|CaptchaPic4.bmp|CaptchaPic4.bmp|CaptchaPic5.bmp|CaptchaPic6.bmp|CaptchaPic7.bmp|CaptchaPic8.bmp|CaptchaPic9.bmp|CaptchaPic10.bmp";
	string captcha2 = "CaptchaPic11.bmp|CaptchaPic12.bmp|CaptchaPic13.bmp|CaptchaPic14.bmp|CaptchaPic15.bmp|CaptchaPic16.bmp|CaptchaPic17.bmp|CaptchaPic18.bmp|CaptchaPic19.bmp|CaptchaPic20.bmp";
	string captcha3 = "CaptchaPic21.bmp|CaptchaPic22.bmp|CaptchaPic23.bmp|CaptchaPic24.bmp|CaptchaPic25.bmp|CaptchaPic26.bmp|CaptchaPic27.bmp|CaptchaPic28.bmp|CaptchaPic29.bmp|CaptchaPic30.bmp";
	string catpcha4 = "CaptchaPic31.bmp|CaptchaPic32.bmp|CaptchaPic33.bmp|CaptchaPic34.bmp|CaptchaPic35.bmp|CaptchaPic36.bmp|CaptchaPic37.bmp|CaptchaPic38.bmp|CaptchaPic39.bmp|CaptchaPic40.bmp";
	string captcha5 = "CaptchaPic41.bmp|CaptchaPic42.bmp|CaptchaPic43.bmp|CaptchaPic44.bmp|CaptchaPic45.bmp|CaptchaPic46.bmp|CaptchaPic47.bmp|CaptchaPic48.bmp|CaptchaPic49.bmp|CaptchaPic50.bmp";
	pics.append(captcha1);
	pics.append(captcha2);
	pics.append(captcha3);
	pics.append(catpcha4);
	pics.append(captcha5);

	bstr = DM->PskFindPicE(180, 110, 415, 280, pics.c_str(), "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "CaptchaPicX=" << result[1].c_str();
	qDebug() << "CaptchaPicY=" << result[2].c_str();
...
int TianxiaScript::tianlaoZhiyuan(Ifire* DM)
{
	...
	if (findTianlaoZhiyuan(DM) == 1) {
		...
		while (getIsEnded() == false) {
			...
			if (IsInTianlao(DM) == false) {
				break;
			}

			if (baoxiangLingqu(DM) == 1) {
				break;
			}
		}
	}
...
int TianxiaScript::findLingshouWnd(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(250, 5, 350, 150, "LingshouWnd.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "LingshouWndX=" << result[1].c_str();
	qDebug() << "LingshouWndY=" << result[2].c_str();
...
int TianxiaScript::findWeishiWnd(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(240, 20, 350, 150, "WeishiWnd.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "WeishiWndX=" << result[1].c_str();
	qDebug() << "WeishiWndY=" << result[2].c_str();
...
{
	if (this->findWeishiWnd(DM) == 1) {
		DM->PskMToEx(490, 55, 3, 3);
		DM->PskLClick();
	}
...
int TianxiaScript::huoliWuxingIn(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(100, 60, 520, 300, "HuoliWuyao.bmp|HuoliWuxing.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::clickSysIcon(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(490, 100, 600, 280, "SysIcon.bmp|SysIcon2.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::changguiTask(Ifire* DM)
{
	...
	if (currentTask == "HDMP") {
		...
		table->setItem(index - 1, 2, new QTableWidgetItem(QString::fromLocal8Bit("回到门派")));
		if (huidaoMenpai(DM) == 1) {
			this->scriptIndex++;
		}
		else {
			tempCounter++;
	...
	if (currentTask == "WMA") {
		...
		table->setItem(index - 1, 2, new QTableWidgetItem(QString::fromLocal8Bit("喂马")));
		if (weima(DM) == 1) {
			this->scriptIndex++;
		}
		else {
	...
	if (currentTask == "LGJZ") {
		...
		if (this->liuguangPrefix > currentTime || this->liuguangSuffix <  currentTime || this->currentLevel<60) {
			scriptIndex++;
			return 0;
		}

		table->setItem(index - 1, 2, new QTableWidgetItem(QString::fromLocal8Bit("流光军姿")));
		if (liuguangJunziEx(DM) == 2) {
			tempCounter++;
		}
		else {
			tempCounter = 0;
	...
	if (currentTask == "JOIS") {
		...
		if (findShiliWnd(DM) == 0) {
			...
			QString forceId = this->forceIdField;
			if (forceId.length() > 0 && forceId != "0") {
				qDebug() << "forceId = " << forceId;
				if (findShiliWnd(DM) == 1) {
					DM->PskMTo(300, 310);
					DM->PskLClick();
					Sleep(1000);
					this->sendString(forceId);
					Sleep(2000);
					DM->PskMTo(200, 100);
					DM->PskLClick();
					Sleep(1000);
	...
	if (currentTask == "CJSL") {
		qDebug() << "CJSL";
		table->setItem(index - 1, 2, new QTableWidgetItem(QString::fromLocal8Bit("创建势力")));
		...
		if (findShiliWnd(DM) == 0) {
			...
			if (findShiliWnd(DM) == 1) {
				...
				if (findCreateForcePanel(DM) == 1) {
					DM->PskMTo(195, 110);
					DM->PskLClick();
					Sleep(1000);

					for (int i = 0; i < 5; i++) {
						sendStr(CommonUtil::getRandomCNChar());
						Sleep(1000);
					}
					Sleep(1000);
					DM->PskMTo(460, 110);
	...
	if (currentTask == "WYP") {
		qDebug() << "WYP";
		table->setItem(index - 1, 2, new QTableWidgetItem(QString::fromLocal8Bit("妖五行盘")));
		wuxingCounter++;
		if (wuxingCounter > 30) {
			map2Jiuli(DM);
			scriptIndex++;
	...
	if (currentTask == "CHLL") {
		...
		newsClose(DM);
		shangchengClose(DM);
		table->setItem(index - 1, 2, new QTableWidgetItem(QString::fromLocal8Bit("淬火琉璃")));
		if (submitLiuli(DM) == 0) {
	...
	if (currentTask == "JDQS") {
		qDebug() << "JDQS";
		//table->item(index - 1, 2)->setText("吃基");
	...
	if (currentTask == "KLJB") {
		qDebug() << "KLJB";
		table->setItem(index - 1, 2, new QTableWidgetItem(QString::fromLocal8Bit("快驴加鞭")));
		DM->PskKPress(27);
		tuichuDuiwu(DM);

		if (kuaiLvJiabianEx(DM) == 2) {
...
int TianxiaScript::clickWuxingInPanel(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(70, 50, 180, 300, "WuxingpanInPanel.bmp|WuxingpanInPanel2.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "WuxingpanInPanelX=" << result[1].c_str();
	qDebug() << "WuxingpanInPanelY=" << result[2].c_str();
...
int TianxiaScript::clickRunInPanel(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(400, 200, 550, 360, "RunInPanel.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "RunInPanelX=" << result[1].c_str();
	qDebug() << "RunInPanelY=" << result[2].c_str();
...
int TianxiaScript::clickGongzhanCloseBtn(Ifire* DM)
{
	...
	bstr = DM->PskFindPicE(220, 10, 360, 150, "lapiaoWnd.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "lapiaoWndX=" << result[1].c_str();
	qDebug() << "lapiaoWndY=" << result[2].c_str();

	if (x != -1) {

		DM->PskMToEx(516, 35, 5, 3);
		DM->PskLClick();
		Sleep(1000);
		DM->PskMToEx(520, 40, 5, 3);
		DM->PskLClick();
		Sleep(1000);
		return 1;
	}
...
int TianxiaScript::findCreateForcePanel(Ifire* DM)
{
	...
	bstr = DM->PskFindPicE(220, 30, 350, 150, "CreateForcePanel.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "CreateForcePanelX=" << result[1].c_str();
	qDebug() << "CreateForcePanelY=" << result[2].c_str();
...
int TianxiaScript::findLiuguangJunzi(Ifire* DM)
{
	...
	bstr = DM->PskFindPicE(55, 80, 550, 350, "LiuguangJunziTitle2.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "LiuguangJunziTitleX=" << result[1].c_str();
	qDebug() << "LiuguangJunziTitleY=" << result[2].c_str();
...
int TianxiaScript::clickJunziGaoji(Ifire* DM)
{
	...
	bstr = DM->PskFindPicE(420, 80, 600, 250, "JunziGaoji.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "JunziGaojiX=" << result[1].c_str();
	qDebug() << "JunziGaojiY=" << result[2].c_str();
...
int TianxiaScript::clickQingbaoZLBtn(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(280, 200, 400, 300, "QingbaoZLBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "QingbaoZLBtnX=" << result[1].c_str();
	qDebug() << "QingbaoZLBtnY=" << result[2].c_str();
...
int TianxiaScript::cuncangSubScript(Ifire* DM)
{
	...
	for (int j = 0; j < 5 && getIsEnded() == false; j++) {
		for (int i = 0; i < 5 && getIsEnded() == false; i++) {
			...
			else if (cuncang4TimeLimitedItem(DM) == 1) {
...
int TianxiaScript::qingliBaoguoSubScript(Ifire* DM)
{
	...
	for (int j = 0; j < 5 && getIsEnded() == false; j++) {
		for (int i = 0; i < 5 && getIsEnded() == false; i++) {
			...
			else if (qingbaoSub4ZhuangbeiLihe(DM) == 1) {
...
int TianxiaScript::clickLiuliItem(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(305, 80, 525, 350, "LiuliItem.bmp|LiuliItem2.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "LiuliItemX=" << result[1].c_str();
	qDebug() << "LiuliItemY=" << result[2].c_str();
...
int TianxiaScript::clickYouxiaZhuangbeiBtn(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(450, 250, 560, 360, "YouxiaZhuangbeiBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "YouxiaZhuangbeiBtnX=" << result[1].c_str();
	qDebug() << "YouxiaZhuangbeiBtnY=" << result[2].c_str();

	if (x != -1) {
		DM->PskMToEx(x + 5, y + 5, 5, 5);
		DM->PskLClick();
		Sleep(1000);
...
int TianxiaScript::submitLiuliItem(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(460, 180, 550, 300, "SubmitLiuliBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "SubmitLiuliBtnX=" << result[1].c_str();
	qDebug() << "SubmitLiuliBtnY=" << result[2].c_str();
...
int TianxiaScript::submitWndClose(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(350, 150, 475, 300, "LiuliSubmit.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "LiuliSubmitX=" << result[1].c_str();
	qDebug() << "LiuliSubmitY=" << result[2].c_str();
...
int TianxiaScript::qingbaoSub4Gengduo(Ifire* DM)
{
	...
	bstr = DM->PskFindPicE(280, 150, 540, 350, "QingbaoGengduo.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::qingbaoSub4BiwuJiajiang(Ifire* DM)
{
	...
	bstr = DM->PskFindPicE(80, 100, 300, 200, "BiwuJiajiang.bmp|Libao.bmp|QingbaoLiuli.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::qingbaoSub4ZhuangbeiLihe(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(50, 100, 300, 200, "40Libao.bmp|30Libao.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::cuncangSub4Yuezuan(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(80, 100, 300, 200, "QingbaoYuezuan.bmp|Zhizuoshu.bmp|QuandeKuizen.bmp|DuiZhangLibao1.bmp|DuizhangLibao2.bmp|Fuyunjiaozi.bmp|40Libao.bmp|30Libao.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::cuncangSub4Rizuan(Ifire* DM)
{
	...
	bstr = DM->PskFindPicE(80, 100, 300, 200, "QingbaoRizuan.bmp|Huilingdan.bmp|Jingyanfu.bmp|Libao.bmp|JiuJiu.bmp|Zizhidan.bmp|DuiZhangLibao1.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::cuncang4TimeLimitedItem(Ifire* DM)
{
	...
	std::vector<std::string> result;
	//bstr = DM->PskFindPicE(45, 120, 130, 270, "禁止交易.bmp|JinzhiJiaoyi.bmp", "000000", 0.9, 0);
	bstr = DM->PskFindPicE(40, 120, 180, 300, "GuoqiShijian.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "GuoqiShijianX=" << result[1].c_str();
	qDebug() << "GuoqiShijianY=" << result[2].c_str();
...
int TianxiaScript::qingbaoSub4Weiling(Ifire* DM)
{
	...
	bstr = DM->PskFindPicE(280, 20, 540, 200, "QingbaoWeiling.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::findBeibao(Ifire* DM)
{
	...
	qDebug() << "BeibaoQuanbuX=" << result[1].c_str();
	qDebug() << "BeibaoQuanbuY=" << result[2].c_str();
...
{
	/*while (clickKuailvJianbian(DM) == 1 && getIsEnded() == false) {
		DM->PskKPress(27);
		Sleep(29000);
	}*/
	/*while (clickKuailvJianbian(DM) == 1 && getIsEnded() == false) {
		DM->PskKPress(27);
		Sleep(1000);
		while ((dianjiTiaoguo(DM) == 1 || duihuaHuangtiao(DM) == 1) && getIsEnded() == false) {
			Sleep(1000);
			clickKuailvCenterPanel(DM);
	...
	bstr = DM->PskFindPicE(70, 70, 200, 350, "KuailvIn1.bmp|KuailvIn2.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	bstr = DM->PskFindPicE(400, 200, 550, 360, "JieshouIn.bmp|JieshouIn2.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::clickKuailvJianbian(Ifire* DM) {
	...
	if (findChangguiIn(DM) == 1) {
		if (clickKuailvTitle(DM) == 1) {
			DM->PskMToEx(470, 310, 10, 5);
			DM->PskLClick();
			Sleep(1000);
...
int TianxiaScript::clickKuailvTitle(Ifire* DM)
{
	...
	qDebug() << "KuailvIn1X=" << result[1].c_str();
	qDebug() << "KuailvIn1Y=" << result[2].c_str();

	if (x != -1) {

		DM->PskMToEx(x + 2, y + 2, 10, 5);
		DM->PskLClick();
		Sleep(1000);
		return 1;
	}
...
int TianxiaScript::clickKuailvCenterPanel(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(200, 100, 400, 250, "CenterPanel.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "CenterPanelX=" << result[1].c_str();
	qDebug() << "CenterPanelY=" << result[2].c_str();
...
int TianxiaScript::jieriClose(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(520, 0, 600, 100, "JieriClose.bmp|JiaoziClose.bmp|LvrenJieriCloseBtn.bmp|SakuraCloseBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
int TianxiaScript::lvrenBaoxiangClose(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(200, 200, 400, 400, "ZhizuobaoxiangBtn.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
	...
	qDebug() << "ZhizuobaoxiangBtnX=" << result[1].c_str();
	qDebug() << "ZhizuobaoxiangBtnY=" << result[2].c_str();

	if (x != -1) {

		DM->PskMToEx(565, 15, 5, 5);
		DM->PskLClick();
		Sleep(1000);
...
int TianxiaScript::findLoginScreen(Ifire* DM)
{
	...
	std::vector<std::string> result;
	bstr = DM->PskFindPicE(250, 200, 450, 350, "点击换服.bmp|DianjiHuanfu.bmp", "000000", 0.9, 0);
	str = (_bstr_t)bstr;
...
void TianxiaScript::getConfig(QString config) {
	...
	if (!document.isNull() && (jsonError.error == QJsonParseError::NoError)) {
		if (document.isObject()) {
			...
			//mitan_counter
			if (object.contains("forceIdField")) {
				QJsonValue value = object.value("forceIdField");
				if (value.isString()) {
					QString strName = value.toString();
					qDebug() << "forceIdField : " << strName;
					forceIdField = strName;
				}
			}
			//shimen_counter
...
unsigned __stdcall TianxiaScript::ThreadFun(void * pParam) {
	...
	try {
		...
		script.juediCounter = 0;
		script.isEquiped = false;
		...
		todayStr = CommonUtil::getCurrentDateStr(0);
		todayStr.append(" 10:00:00");
		int liuguangPrefixTime = CommonUtil::GetTimeStampByStr(todayStr.c_str());

		todayStr = CommonUtil::getCurrentDateStr(0);
		todayStr.append(" 23:00:00");
		int liuguangSuffixTime = CommonUtil::GetTimeStampByStr(todayStr.c_str());


		script.liuguangPrefix = liuguangPrefixTime;
		script.liuguangSuffix = liuguangSuffixTime;
		...
		while (script.getIsEnded() == false) {
			...
			//DM->PskCapture(0, 0, 300, 300, "D:\\test.jpg");
			...
			//20180510ST
			script.clickFuyunCloseBtn(DM);
			script.clickJijinCloseBtn(DM);
			//20180510EN
